...
\begin{tikzpicture}[node distance=4mm]
  ...
  \node[box=19mm] (cdm) at (0.6,0.6) {Microsoft CDM\\ 42 JSON files};
  \node[box=21mm, right=of cdm] (parse) {Parser\\ inheritance + manifest relationships};
  \node[box=19mm, right=of parse] (ent) {27 entities\\ $\rightarrow$ 168 chunks};
  \node[box=19mm, right=of ent] (emb) {Embeddings\\ bge-small-en-v1.5};
  \node[box=19mm, right=of emb] (db) {Chroma\\ vectors + metadata};
  \draw[flow] (cdm) -- (parse);
  \draw[flow] (parse) -- (ent);
  \draw[flow] (ent) -- (emb);
  \draw[flow] (emb) -- (db);
  ...
  \node[box=19mm] (api) at (0.6,-2.5) {FastAPI\\ \code{POST /ask}};
  \node[box=21mm, right=of api] (ret) {Retriever\\ name match + top-5, score $\ge$ 0.63};
  \node[box=19mm, right=of ret] (ctx) {Numbered context\\ + source paths};
  \node[box=19mm, right=of ctx] (llm) {OpenAI\\ gpt-4.1-mini, temp.~0};
  \node[box=19mm, right=of llm] (ans) {Answer\\ + \code{[n]} citations + sources};
  \draw[flow] (api) -- (ret);
  \draw[flow] (ret) -- (ctx);
  \draw[flow] (ctx) -- (llm);
  \draw[flow] (llm) -- (ans);
